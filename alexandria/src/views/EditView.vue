<template>
    ...
    <template v-else-if="asset.type == 'render'">
        <EditRender
        :asset="asset"
        @update="updateAsset"
        />
    ...
    <template v-else-if="asset.type == 'proc_mat'">
        <EditProcMat
        :asset="asset"
        @update="updateAsset"
        />
...
export default {
    ...
    methods: {
        async updateAsset(updated) {
            ...
            currLib[toUpdate].settings = await this.copyFile(updated.settings)
            try {
        ...
        async copyFile(fpath) {
            if (fpath != "") {
                let new_path = await window.alexandria.copyFile(fpath, this.asset.path)
                return new_path
            }
        }
    },
